...
func isItNow(_ date: Date) -> Bool {
    abs(date.timeIntervalSinceNow) < abs(Date.now.timeIntervalSince(Date.init(timeInterval: 3600, since: Date.now)))
}

print(isItNow(Date.now.addingTimeInterval(3700)))

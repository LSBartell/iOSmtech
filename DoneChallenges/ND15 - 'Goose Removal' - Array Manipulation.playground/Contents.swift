...
func gooseRemoval(from animalList: [String], alsoRemove additionalRemoveList: [String]) -> [String] {
    var mutableAnimalList = animalList
    for animal in mutableAnimalList {
        if geese.contains(animal) || additionalRemoveList.contains(animal) {
            mutableAnimalList.removeAll(where: {$0 == animal})
        }
    }
    return mutableAnimalList
}

print(gooseRemoval(from: ["Mallard", "Hook Bill", "African", "Crested", "Pilgrim", "Toulouse", "Blue Swedish"], alsoRemove: ["Crested", "Blue Swedish"]))

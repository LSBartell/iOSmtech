...
func ateFor(_ input: String) -> Meal? {
    if let meal = meals[input] {
        return meal
    } else {
        return nil
    }
...
func mealLog() -> [String: Any] {
    if let saved = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return saved
    } else {
        return [String: Any]()
    }
